...
import { mockWorkout, exerciseLibrary } from "@/lib/mock-data";
        ...
        <div className="lg:col-span-2 space-y-4">
          <div className="rounded-2xl border border-border bg-card p-5 shadow-card">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-xs uppercase tracking-wide text-muted-foreground">{mockWorkout.type}</p>
                <h3 className="text-lg font-semibold">{mockWorkout.name}</h3>
              </div>
        ...
        <div className="rounded-2xl border border-border bg-card p-5 shadow-card">
          ...
          <ul className="space-y-2">
            {exerciseLibrary.map((e) => (
              <li key={e.id} className="flex items-center justify-between rounded-lg border border-border bg-background p-3">
                <div>
                  <p className="text-sm font-medium">{e.name}</p>
                  <p className="text-xs text-muted-foreground">{e.type} · {e.equipment}</p>
                </div>
                <button className="rounded-md border border-border bg-card px-2.5 py-1 text-xs font-medium hover:bg-muted">
                  Add
                </button>
              </li>
            ))}
